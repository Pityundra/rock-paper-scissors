...
def _get_skin_likeliness(img: np.ndarray, target_red, target_blue, max_tol_red, max_tol_blue, imshow=False):
    ...
    b = cv2.divide(b, rgb)
    r = cv2.divide(r, rgb)
    # Megjelenitjük a relativ vörös és relativ kék értékeket képként,
    # ezzel ellenőrizhető a beállitott értékek helyessége
    if imshow:
        cv2.imshow('relative r+b', cv2.merge(((b * 255).astype(np.uint8),
...
def _get_skin_regions(img: np.ndarray, imshow=False):
    likeliness = _get_skin_likeliness(img, 160, 40, 70, 36, imshow)
    if imshow:
        cv2.imshow('likeliness', likeliness)
